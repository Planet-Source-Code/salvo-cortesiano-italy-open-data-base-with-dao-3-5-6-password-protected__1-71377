...
Public Const HH_ADD_NAV_UI = &H10&
Public Const HH_ADD_BUTTON = &H11&
Public Const HH_GETBROWSER_APP = &H12&
...
Public Const HH_GET_LAST_ERROR = &H20&
...
Public Function DialogFile(hWnd As Long, wMode As Integer, szDialogTitle As String, szFilename As String, szFilter As String, szDefDir As String, szDefExt As String) As String
    Dim x As Long, ofns As OPENFILENAME, szFile As String, szFileTitle As String
    ofns.lStructSize = Len(ofns)
    ofns.hWnd = hWnd
    ofns.lpstrTitle = szDialogTitle
    ofns.lpstrFile = szFilename & String$(250 - Len(szFilename), 0)
    ofns.nMaxFile = 255
    ofns.lpstrFileTitle = String$(255, 0)
    ofns.nMaxFileTitle = 255
    ofns.lpstrFilter = szFilter
    ofns.nFilterIndex = 1
    ofns.lpstrInitialDir = szDefDir
    ofns.lpstrDefExt = szDefExt
    If wMode = 1 Then
        ofns.flags = OFN_HIDEREADONLY Or OFN_PATHMUSTEXIST Or OFN_FILEMUSTEXIST
        x = GetOpenFileName(ofns)
    Else
        ofns.flags = OFN_HIDEREADONLY Or OFN_OVERWRITEPROMPT Or OFN_PATHMUSTEXIST
        x = GetSaveFileName(ofns)
    End If
    If x <> 0 Then
        If InStr(ofns.lpstrFile, Chr$(0)) > 0 Then
            szFile = Left$(ofns.lpstrFile, InStr(ofns.lpstrFile, Chr$(0)) - 1)
        End If
        DialogFile = szFile
    Else
        DialogFile = ""
